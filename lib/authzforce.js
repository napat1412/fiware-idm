...
const config = require('../config.js').authorization.authzforce;
...
// Function to delete authzforce domain of application
exports.destroy_application_domain = function (domain) {
  debug('--> destroy_application_domain');

  return new Promise(function(resolve, reject) {
    // if application has an authzforce domain assigned, remove an authzforce domain
    if (domain.az_domain) {
        ...
        function(error, body) {
          if (!error) {
            ...
            try {
              ...
              debug('---> skip authorization header configuration: '+error);
            }
            ...
            const options = {
              host: config.host,
              port: config.port,
              path: '/authzforce-ce/domains/' + domain.az_domain,
              method: 'DELETE',
              headers,
            };
            ...
            const req = http.request(options, function(response) {
              ...
              response.on('end', function() {
                if (response.statusCode === 200) {                  
                  debug('Remove Authzforce domain: success');
                  resolve(response.statusCode);
                } else {
                  debug('Remove Authzforce domain: failed');
            ...
            req.on('error', function(error) {
              debug('Error: ', error);
              reject('Remove domain: connection with authzforce failed');
            });
...
function policy_set_update(
  ...
  return new Promise(function(resolve, reject) {
    ...
    ejs.renderFile(
      path.join(__dirname, '/../templates/authzforce/policy_set.ejs'),
      context,
      function(error, body) {
        if (!error) {
          // Set headers
      ...
      function(error, body) {
        if (!error) {
          ...
          req.on('error', function(error) {
            debug('Error: ', error);
            reject(
              'Activate policy: connection with authzforce failed' + error
            );
          });
...
exports.check_connection = function() {
  ...
  return new Promise(function(resolve, reject) {
    ...
    req.on('error', function(error) {
      debug('Error: ', error);
      reject(
        'Error: connection with Authzforce failed. Policies will not  be created'
      );
    });
