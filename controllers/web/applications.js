...
exports.step_create_avatar = function(req, res) {
  ...
  if (req.file) {
    handle_uploaded_images(
      req,
      res,
      '/idm/applications/' + req.application.id + '/step/roles'
    );
...
exports.change_token_type = function(req, res) {
  ...
  if (
    token_types.length <= 0 ||
    token_types.every(r => allowed_types.includes(r))
  ) {
    const jwt_secret = token_types.includes('jwt')
      ? crypto
          .randomBytes(16)
          .toString('hex')
          .slice(0, 16)
...
function delete_image(req, res, image_path, success, redirect_uri, message) {
  image
    ...
    .then(function() {
      ...
      res.redirect(
        success ? redirect_uri : '/idm/applications/' + req.application.id
      );
    })
    .catch(function(error) {
      debug('Error: ', error);
